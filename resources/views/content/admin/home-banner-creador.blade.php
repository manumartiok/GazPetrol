...
@section('content')
   ...
   {{-- cabezal --}}
    <div>
        <h3 class="text-[20px] font-bold text-gray-500">Crear banner Home</h3>
        <hr class="mx-6">
    </div>
    ...
    <div class="mx-20 pt-6" id="app">
        <div class="border border-rounded p-4">
            <div>
                <h3 class="text-[18px] font-semibold  text-gray-600">Crear banner</h3>
            </div>
            <hr class="my-3">
            
            <form method="POST" action="{{ route('adm.home-ban-store') }}" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="home_banner_id" value="{{ $home_banner->id ?? '' }}">
